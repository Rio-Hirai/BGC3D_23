...
public class Sample_L : MonoBehaviour
{
    ...
    void AddALine()
    {
        ...
        Color color = Color.HSVToRGB((h += 0.1f) > 1 ? (h - 1) : 0.2f, 0.8f, 0.8f);
    ...
    private void ContinueInput(float f)
    {
        ...
        {
            if (script.LeftPupiltDiameter_flag == 1)
            {
                m_DataDiagram.InputPoint(l, new Vector2(0.1f, script.LeftPupiltDiameter));
            } else
            {
                m_DataDiagram.InputPoint(l, new Vector2(0.1f, 0.0f));
            }
            
            //m_DataDiagram.InputPoint(l, new Vector2(0.1f, script.RightPupiltDiameter));
